...
import type { Earthquake, SwarmEvent } from './types';
...
export async function generateActivitySummary(input: ActivitySummaryInput, apiKey?: string): Promise<string | null> {
...
export async function generateEarthquakeExplanation(input: EarthquakeExplanationInput, apiKey?: string): Promise<string | null> {
  const client = getOpenAIClient(apiKey);
  if (!client) return null;
...
export async function getCachedActivitySummary(input: ActivitySummaryInput, apiKey?: string): Promise<string | null> {
  ...
  const summary = await generateActivitySummary(input, apiKey);
  if (summary) {
