...
extension Earthquake {
    ...
    /// Create an Earthquake from API feature data
    static func from(feature: APIResponse.Feature) -> Earthquake {
        let coords = feature.geometry.coordinates
        let timestampSeconds = TimeInterval(feature.properties.time) / 1000.0
        
        return Earthquake(
            id: feature.id,
            magnitude: feature.properties.mag ?? 0,
            place: feature.properties.place ?? "Unknown location",
            time: Date(timeIntervalSince1970: timestampSeconds),
            timestamp: TimeInterval(feature.properties.time),
            ...
            regionId: "unknown" // Will be calculated by RegionService
        )
    }
}
