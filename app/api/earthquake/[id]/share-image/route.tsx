...
/**
 * GET /api/earthquake/[id]/share-image
 * 
 * Returns the earthquake share card image optimized for iOS sharing.
 * Supports query params:
 * - format: 'png' (default) or 'jpeg'
 * - size: 'standard' (1200x630) or 'square' (1080x1080) for Instagram
 */
export async function GET(
  ...
  const sizeType = searchParams.get('size') || 'standard';
  ...
  // Size configuration
  const size = sizeType === 'square' 
    ? { width: 1080, height: 1080 }
    : { width: 1200, height: 630 };
  
  const isSquare = sizeType === 'square';
  ...
  const imageResponse = new ImageResponse(
    (
      ...
      >
        ...
        {/* Main content */}
        <div style={{ flex: 1, display: 'flex', alignItems: isSquare ? 'center' : 'center', gap: isSquare ? 32 : 48, flexDirection: isSquare ? 'column' : 'row' }}>
          {/* Magnitude badge */}
          <div
            style={{
              width: isSquare ? 220 : 180,
              height: isSquare ? 220 : 180,
              borderRadius: 28,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, alignItems: isSquare ? 'center' : 'flex-start', textAlign: isSquare ? 'center' : 'left' }}>
            <span style={{ 
            ...
            {locationContext.formattedLocation && earthquake.place && !isSquare && (
            ...
            {/* Details row */}
            <div style={{ display: 'flex', gap: isSquare ? 24 : 32, marginTop: 8, flexWrap: 'wrap', justifyContent: isSquare ? 'center' : 'flex-start' }}>
        ...
        {/* Footer badges */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: isSquare ? 'center' : 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', justifyContent: isSquare ? 'center' : 'flex-start' }}>
            <div
              style={{
                padding: '12px 20px',
                borderRadius: 12,
  ...
  // Return with appropriate headers for sharing
  const headers = new Headers();
  headers.set('Content-Type', format === 'jpeg' ? 'image/jpeg' : 'image/png');
  headers.set('Cache-Control', 'public, max-age=3600, s-maxage=3600');
  headers.set('Content-Disposition', `inline; filename="earthquake-${id}.${format === 'jpeg' ? 'jpg' : 'png'}"`);
